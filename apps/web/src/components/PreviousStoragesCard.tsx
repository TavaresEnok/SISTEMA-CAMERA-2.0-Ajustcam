...
export function PreviousStoragesCard({ apiUrl, accessToken }: { apiUrl: string; accessToken: string | null }) {
  ...
  const anteriores = storages.filter((s) => !s.isActive);
  // Sem storage anterior não há nada a administrar; a seção inteira some em vez
  // de ocupar espaço dizendo "vazio".
    ...
    <div className="space-y-3 p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="text-sm">
          <div className="font-medium">Storages anteriores</div>
          <p className="mt-0.5 text-xs text-muted-foreground">
            Não recebem gravação nova. O acervo continua sendo lido de onde foi gravado e
            se esvazia sozinho conforme a retenção vence — o que está abaixo é para quem não quer esperar.
          </p>
        </div>
      </div>
